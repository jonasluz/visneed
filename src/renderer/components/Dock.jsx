...
function Dock({currentPage, treeId, treeName, onHomeClick }) {
  ...
  return (
    <div className='flex flex-row justify-around absolute bottom-0 z-40 left-1/2 transform -translate-x-1/2 -translate-y-1/2 right- w-[17%] h-[8%] bg-background-green-200 rounded-lg p-2 border items-center'>
        <div
        className='lg:w-8 lg:h-8 xl:w-10 xl:h-10 2xl:w-12 2xl:h-12 p-3 hover:bg-background-green-400 duration-100 ease-in rounded-lg cursor-pointer'
        onClick={() => {handleClickDock('home')}}>
          <img src={houseIcon} alt="" draggable={false}/>
        </div>
        
        <div 
        className={`lg:w-8 lg:h-8 xl:w-10 xl:h-10 2xl:w-12 2xl:h-12 p-3 hover:bg-background-green-400 ${currentPage == 'dictionary' ? 'bg-background-green-400' : ''} duration-100 ease-in rounded-lg cursor-pointer`}
        onClick={() => {handleClickDock('dictionary')}}>
          <img src={dictionaryIcon} alt="" draggable={false}/>
        </div>
        
        <div 
        className={`lg:w-8 lg:h-8 xl:w-10 xl:h-10 2xl:w-12 2xl:h-12 p-3 hover:bg-background-green-400 ${currentPage == 'treePage' ? 'bg-background-green-400' : ''} duration-100 ease-in rounded-lg cursor-pointer`}
        onClick={() => {handleClickDock('treePage')}}>
          <img src={treeIcon} alt="" draggable={false}/>
        </div>
        
        <div
        className={`lg:w-8 lg:h-8 xl:w-10 xl:h-10 2xl:w-12 2xl:h-12 p-3 hover:bg-background-green-400 ${currentPage == 'cenario' ? 'bg-background-green-400' : ''} duration-100 ease-in rounded-lg cursor-pointer`}
