...
function Dock({currentPage, treeId, treeName, onHomeClick }) {
  ...
  return (
    <div className='flex flex-row justify-around absolute bottom-0 z-50 left-1/2 transform -translate-x-1/2 -translate-y-1/2 right- w-[17%] h-[8%] bg-background-green-200 rounded-lg p-2 border items-center'>
        <div
        className='w-14 h-14 p-3 hover:bg-background-green-400 duration-100 ease-in rounded-lg cursor-pointer'
        onClick={() => {handleClickDock('home')}}>
          <img src={houseIcon} alt="" draggable={false}/>
        </div>
        
        <div 
        className={`w-14 h-14 p-3 hover:bg-background-green-400 ${currentPage == 'dictionary' ? 'bg-background-green-400' : ''} duration-100 ease-in rounded-lg cursor-pointer`}
        onClick={() => {handleClickDock('dictionary')}}>
          <img src={dictionaryIcon} alt="" draggable={false}/>
        </div>
        
        <div 
        className={`w-14 h-14 p-3 hover:bg-background-green-400 ${currentPage == 'treePage' ? 'bg-background-green-400' : ''} duration-100 ease-in rounded-lg cursor-pointer`}
